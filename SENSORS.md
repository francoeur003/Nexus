# Nexus Sensor Reference

Complete map of every hardware sensor used by Nexus v2.0, verified live on an **Apple M2** (MacBook Air, fanless).  
...
> **Note:** Nexus uses `TCMz` as the authoritative "CPU Die Hotspot" because it reacts
...
> Nexus uses `PSTR` directly for the System Power readout.  
...
> On M5+ chips, `AMC Stats` is kernel-blocked. Nexus falls back to `PMP` group
...
Nexus reads `F0Ac` and exposes it as `fanRPM`. The UI hides this section when `fanRPM == 0`.
...
Nexus exposes these in debug mode only. Included here for contributors.
...
Nexus infers the chip variant (base / Pro / Max / Ultra) from:
...
| Metric | Nexus (native) | mactop | Delta | Status |
|--------|---------------------|--------|-------|--------|
